...
export function resolvePathString (respField, respFieldsPaths, obj = self, separator = '/') {
  ...
  try {
    const path = respFieldsPaths[respField].path
    ...
    const dataObject = objectFromPath(obj, path, separator)
...
export function searchItems (endpointGenerated = undefined, endpointRawConfig = undefined) {
  ...
  const fetchMethod = endpointRawConfig.method
  console.log('+ + + searchItems / fetchMethod : ', fetchMethod)

  const fetchHeader = endpointGenerated.requestHeader
  // fetchHeader = {}
  ...
  const fetchUrl = endpointGenerated.requestUrl
  ...
  const fetchPayload = endpointGenerated.requestPayload
  ...
  const payloadJson = JSON.stringify(fetchPayload)
  ...
  return {
    abort () {
      searchAborted = true
      if (ac) { console.log('+ + + searchItems / (axios) / abort ac :', ac) }
      ac.abort()
    },
    promise: axios(
      axiosOptions
      ...
      .then(resp => {
        ...
        const responseProjects = resolvePathString('projects', responsePaths, resp.data, '/')
        ...
        if (endpointRawConfig.has_resp_callbacks) {
          ...
          const callbacks = endpointRawConfig.resp_callbacks
          console.log('+ + + searchItems / (axios) / callbacks : ', callbacks)
        }

        const responseTotal = resolvePathString('total', responsePaths, resp.data, '/')
        console.log('+ + + searchItems / (axios) / responseProjects : ', responseProjects)

        const responseStats = resolvePathString('stats', responsePaths, resp.data, '/')
...
export function buildRequestHeader (token, endpointConfigHeaderAuth, endpointConfig = undefined) {
  ...
  const endpointHeaderOptions = endpointConfig.request_header_options
...
export function buildRequestPayload (endpointConfig) {
  ...
  const payload = endpointConfig.payload_queries
...
export function searchEndpointGenerator (obj) {
  ...
  const fetchMethod = endpointConfig.method
  ...
  const fetchPayloadOptions = endpointConfig.payload_options
  ...
  const argsArray = []
  for (const key in endpointConfigArgs) {
  ...
  if (filterMapper && selectedFilters.length > 0) {
    ...
    for (const index in selectedFilters) {
      const argFilterString = EndpointArg + '=' + selectedFilters[index]
  ...
  if (argsArray.length > 0) {
    baseQuery += '?'
    const argsLongString = argsArray.join('&')
  ...
  const header = buildRequestHeader(accessToken, endpointConfigHeaderAuth, endpointConfig)

  // build payload from endpointConfig
  const payload = fetchPayloadOptions && buildRequestPayload(fetchPayloadOptions)

  const endPointUrlAndPayload = {
...
export function trimString (string, trimming) {
  const trim = (trimming && string.length > trimming) ? trimming : string.length
