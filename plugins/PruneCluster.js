...
(function (PruneCluster_1) {
  ...
  var Cluster = (function (_super) {
    ...
    Cluster.prototype.AddMarker = function (marker) {
      ...
      var weight = marker.weight; var currentTotalWeight = this.totalWeight; var newWeight = weight + currentTotalWeight
    ...
    Cluster.prototype.ComputeBounds = function (cluster) {
      ...
      var nbX = Math.floor(proj.x / size); var nbY = Math.floor(proj.y / size); var startX = nbX * size; var startY = nbY * size
      var a = cluster.UnProject(startX, startY); var b = cluster.UnProject(startX + size, startY + size)
      this.bounds = {
        minLat: b.lat,
        maxLat: a.lat,
        minLng: a.lng,
    ...
    Cluster.prototype.ApplyCluster = function (newCluster) {
      ...
      var weight = newCluster.totalWeight; var currentTotalWeight = this.totalWeight; var newWeight = weight + currentTotalWeight
      this.averagePosition.lat =
              (this.averagePosition.lat * currentTotalWeight +
  ...
  var PruneCluster = (function () {
    ...
    PruneCluster.prototype._sortMarkers = function () {
      var markers = this._markers; var length = markers.length
    ...
    PruneCluster.prototype._indexLowerBoundLng = function (lng) {
      var markers = this._markers; var it; var step; var first = 0; var count = markers.length
      while (count > 0) {
    ...
    PruneCluster.prototype.ProcessView = function (bounds) {
      var heightBuffer = Math.abs(bounds.maxLat - bounds.minLat) * this.ViewPadding; var widthBuffer = Math.abs(bounds.maxLng - bounds.minLng) * this.ViewPadding
      var extendedBounds = {
      ...
      var markers = this._markers; var clusters = this._clusters
      var workingClusterList = clusters.slice(0)
      for (var i = firstIndex, l = markers.length; i < l; ++i) {
        var marker = markers[i]; var markerPosition = marker.position
        ...
        if (markerPosition.lat > extendedBounds.minLat &&
                  ...
                  !marker.filtered) {
          var clusterFound = false; var cluster
    ...
    PruneCluster.prototype.FindMarkersInArea = function (area) {
      var aMinLat = area.minLat; var aMaxLat = area.maxLat; var aMinLng = area.minLng; var aMaxLng = area.maxLng; var markers = this._markers; var result = []
    ...
    PruneCluster.prototype.ComputeBounds = function (markers, withFiltered) {
      ...
      var rMinLat = Number.MAX_VALUE; var rMaxLat = -Number.MAX_VALUE; var rMinLng = Number.MAX_VALUE; var rMaxLng = -Number.MAX_VALUE
      for (var i = 0, l = markers.length; i < l; ++i) {
...
var PruneClusterForLeaflet = (L.Layer ? L.Layer : L.Class).extend({
  ...
  BuildLeafletCluster: function (cluster, position) {
    ...
    m.on('click', function () {
      ...
      if (b) {
        ...
        var zoomLevelBefore = _this._map.getZoom(); var zoomLevelAfter = _this._map.getBoundsZoom(bounds, false, new L.Point(20, 20))
  ...
  ProcessView: function () {
    ...
    var map = this._map; var bounds = map.getBounds(); var zoom = Math.floor(map.getZoom()); var marginRatio = this.clusterMargin / this.Cluster.Size; var resetIcons = this._resetIcons
    ...
    var objectsOnMap = this._objectsOnMap; var newObjectsOnMap = []; var markersOnMap = new Array(objectsOnMap.length)
    ...
    var workingList = []
    for (i = 0, l = clusters.length; i < l; ++i) {
      var icluster = clusters[i]; var iclusterData = icluster.data
      var latMargin = (icluster.bounds.maxLat - icluster.bounds.minLat) * marginRatio; var lngMargin = (icluster.bounds.maxLng - icluster.bounds.minLng) * marginRatio
      for (var j = 0, ll = workingList.length; j < ll; ++j) {
        ...
        var oldMaxLng = c.averagePosition.lng + lngMargin; var oldMinLat = c.averagePosition.lat - latMargin; var oldMaxLat = c.averagePosition.lat + latMargin; var newMinLng = icluster.averagePosition.lng - lngMargin; var newMinLat = icluster.averagePosition.lat - latMargin; var newMaxLat = icluster.averagePosition.lat + latMargin
      ...
      if (idata._leafletMarker._removeFromMap) {
        ...
        if (marker._zoomLevel === zoom) {
          ...
          for (j = 0, ll = clusterCreationList.length; j < ll; ++j) {
            var jcluster = clusterCreationList[j]; var jdata = jcluster.data
            if (marker._population === 1 && jcluster.population === 1 &&
              ...
              var oldMinLng = pa.lng - lngMargin; var newMaxLng = pb.lng + lngMargin
...
var PruneClusterLeafletSpiderfier = (L.Layer ? L.Layer : L.Class).extend({
  ...
  Spiderfy: function (data) {
    ...
    window.setTimeout(function () {
      ...
      var interval = 42; var duration = 290
      var anim = window.setInterval(function () {
        ...
        for (i = 0, l = points.length; i < l; ++i) {
          var p = projectedPoints[i]; var diffLat = p.lat - center.lat; var diffLng = p.lng - center.lng
  ...
  _generatePointsCircle: function (count, centerPt) {
    var circumference = this.spiderfyDistanceMultiplier * this._circleFootSeparation * (2 + count); var legLength = circumference / this._2PI; var angleStep = this._2PI / count; var res = []; var i; var angle
    res.length = count
    for (i = count - 1; i >= 0; i--) {
  ...
  _generatePointsSpiral: function (count, centerPt) {
    var legLength = this.spiderfyDistanceMultiplier * this._spiralLengthStart; var separation = this.spiderfyDistanceMultiplier * this._spiralFootSeparation; var lengthFactor = this.spiderfyDistanceMultiplier * this._spiralLengthFactor; var angle = 0; var res = []; var i
    res.length = count
    for (i = count - 1; i >= 0; i--) {
...
export function PruneClusterExport () { return PruneCluster } ;
export function PruneClusterForLeafletExport () { return PruneClusterForLeaflet }
