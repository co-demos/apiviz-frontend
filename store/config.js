...
export const getters = {
  ...
  getEndpointConfigAuthSpecific: (state, getters) => (endpointType) => {
    ...
    const allAuthEndpoints = getters.getEndpointConfigAuthUsers
  ...
  getRouteConfigTableForDataset: (state, getters, rootState) => {
    const config = state.config.routes.find(function (r) {
  ...
  getEndpointConfigFilters: (state, getters, rootState) => {
    // state.log && console.log("S-config-getEndpointConfigFilters - state.config.endpoints : \n", state.config.endpoints)
    const config = state.config.endpoints.find(function (r) {
...
export const actions = {
  ...
  checkUuidAuth ({ commit, state, getters, rootGetters }) {
    ...
    const firstArgs = '?auth_mode=' + authMode + '&token=' + userAccessToken

    const fullUrl = rootURLbackend + '/check_uuid/' + apivizFrontUUID + firstArgs
  ...
  getConfigType ({ commit, state, getters, rootGetters }, { type, configTypeEndpoint, args }) {
    ...
    const firstArgs = configTypeEndpoint + '?uuid=' + apivizFrontUUID + '&auth_mode=' + authMode + '&token=' + userAccessToken
    const fullUrl = rootURLbackend + '/config/' + firstArgs + args
    state.log && console.log('S-config-A-getConfigType / fullUrl : ', fullUrl)
  ...
  editConfig ({ commit, state, getters, rootGetters }, request) {
    ...
    // build request URL
    const requestUrl = rootURLbackend + '/config/' + currentColl + '?uuid=' + apivizFrontUUID
    state.log && console.log('S-config-A-editConfig / requestUrl : ', requestUrl)

    // INTERNATIONALIZATION
    if (currentColl === 'global') {
      const appLocales = getters.getAppLocales
  ...
  deleteConfig ({ state, getters, rootGetters }, request) {
    ...
    // build request URL
    const requestUrl = rootURLbackend + '/config/' + currentColl + '/' + docId + '?uuid=' + apivizFrontUUID
  ...
  addConfigDoc ({ commit, state, getters, rootGetters }, request) {
    ...
    // build request URL
    const requestUrl = rootURLbackend + '/add_document/' + currentColl
  ...
  getDefaultApivizModels ({ commit, state, getters, rootGetters }) {
    ...
    // build request URL
    const rootURLbackend = rootGetters.getRootUrlBackend
    const requestUrl = rootURLbackend + '/get_default_models'
    ...
    axios.get(requestUrl)
      ...
      .then(response => {
        ...
        const defaultModels = response.data.models
  ...
  getModelFromUuid ({ state, getters, rootGetters }, uuid) {
    ...
    // build request URL
    const rootURLbackend = rootGetters.getRootUrlBackend
    const requestUrl = rootURLbackend + '/get_config_model/' + uuid
  ...
  createNewConfig ({ state, getters, rootGetters }, request) {
    ...
    // build request URL
    const rootURLbackend = rootGetters.getRootUrlBackend
    const requestUrl = rootURLbackend + '/create_new_config'
