...
export const getters = {
  ...
  getPerPageOptions: (state, getters, rootState) => {
    const defaultPerPage = defaultPagination.perPageOptions
    const endpointPerPage = state.search.endpoint.args_options.find(i => i.app_arg === 'perPage')
  ...
  getGeoResults: (state, getters) => {
    const allResults = getters.getResults
    // console.log(" ++ getGeoResults / allResults : ", allResults)
    if (typeof allResults !== 'undefined') {
      const filtered = allResults.filter(i => !i.lat && !i.lon)
  ...
  getProjectConfigUniform: (state, getters) => (itemData) => {
    ...
    infoTypes.forEach(function (infoType) {
      const fieldObj = getters.getProjectConfig('block_' + infoType)
...
export const mutations = {
  ...
  setIsMapSearch (state, routeConfig) {
    ...
    state.search.question.forMap = (routeConfig.dynamic_template === 'DynamicMap')
  ...
  setQuestionPage (state, pageNumber) {
    ...
    const newPageNumber = state.search.question.page + pageNumber
    if (newPageNumber > 0) {
...
export const actions = {
  ...
  setSearchEndpointConfig ({ state, commit, rootGetters }, currentRouteConfig) {
    ...
    const routeConfig = currentRouteConfig
    ...
    const endpointConfig = rootGetters['config/getEndpointConfig']
    ...
    const endpointPerPage = endpointConfig.args_options.find(i => i.app_arg === 'perPage')
    if (endpointPerPage) {
  ...
  createDatasetFilters ({ state, getters, commit, rootGetters }) {
    ...
    if (currentFiltersConfig && currentFiltersConfig.filter_options) {
      const filterDescriptions = currentFiltersConfig.filter_options
  ...
  search ({ state, commit, dispatch, getters, rootGetters }) {
    ...
    // ENDPOINT GENERATOR
    const endpointGenerated = searchEndpointGenerator({
