...
export default function ({ store, route, redirect }) {
  ...
  const queryIframing = route.query.iframing
  if (queryIframing) {
  ...
  if (typeof currentRouteConfig === 'undefined') {
    ...
    if (DynamicComponents.includes(currentRouteConfig.dynamic_template)) {
      const searchedText = route.query.text
      if (searchedText) {
      ...
      const previousDatasetURI = store.getters['config/getCurrentDatasetURI']
      log && console.log('-M3- getRouteConfig / previousDatasetURI : ', previousDatasetURI)

      const localEndpointConfig = store.getters['config/getEndpointConfig']
      ...
      const currentDatasetURI = localEndpointConfig.dataset_uri
      ...
      if (previousDatasetURI !== currentDatasetURI) {
        ...
        const localFiltersConfig = store.getters['config/getEndpointConfigFilters']
