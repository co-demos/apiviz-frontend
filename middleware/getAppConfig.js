...
export default function ({ store, route, redirect }) {
  ...
  const needAppConfig = !noCheckRoutePaths.includes(route.path)
  log && console.log('-M2- getAppConfig / needAppConfig : ', needAppConfig)

  const isConfigComplete = store.getters['config/getIsConfigComplete']
  log && console.log('-M2- getAppConfig / isConfigComplete : ', isConfigComplete)

  const userAccessToken = store.getters['user/getAccessToken'] !== ''
  log && console.log('-M2- getAppConfig / userAccessToken : ', userAccessToken)

  const isLoginRoutePath = loginRoutes.includes(route.path)
  log && console.log('-M2- getAppConfig / isLoginRoutePath : ', isLoginRoutePath)

  if (needAppConfig) {
    // set up a promise array
    const isConfigUndefined = typeof store.getters['config/getGlobalConfig'] === 'undefined'
    // let isLoginRoute = ( isLoginRoutePath && !isConfigUndefined ) ? isLoginRoutePath : false

    const promiseArrayUuidAuth = []
    const promiseArrayconfigs = []
                ...
                // store.dispatch('config/getConfigAll', isLoginRoute)
                  store.dispatch('config/getConfigAll', isLoginRoutePath)
                    .then(resp => {
                      ...
                      const authUrlRoots = store.getters['config/getEndpointConfigAuthSpecific']('auth_root')
                      ...
                      const appLocales = store.getters['config/getAppLocales']
